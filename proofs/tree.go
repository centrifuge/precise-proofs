...
type TreeOptions struct {
	...
	// IgnoreExcludeFromTree if set true ignores the exclude_from_tree extension
...
func NewDocumentTree(proofOpts TreeOptions) DocumentTree {
	...
	return DocumentTree{
		...
		ignoreExcludeFromTree: proofOpts.IgnoreExcludeFromTree,
	}
}
...
func (doctree *DocumentTree) AddLeavesFromDocument(document proto.Message) (err error) {
	...
	leaves, err := FlattenMessage(
		document,
		getSalt,
		doctree.saltsLengthSuffix,
		doctree.hash,
		doctree.compactProperties,
		doctree.ignoreExcludeFromTree,
		doctree.parentPrefix)
	if err != nil {
		return err
	}
