...
func TestDocumentTree_WithRootHash(t *testing.T) {
	...
	expectedRootHash := []byte{0xa2, 0xe5, 0x54, 0xac, 0xd6, 0x4f, 0x2e, 0xe2, 0x32, 0x42, 0x51, 0x75, 0xdf, 0xbc, 0x1f, 0xae, 0xb1, 0x71, 0x66, 0x98, 0xd2, 0x2e, 0x66, 0x41, 0x40, 0x66, 0xe2, 0x95, 0xc4, 0xe9, 0xef, 0x51}
...
func TestTree_GenerateNestedTreeCombinedStandardProof(t *testing.T) {
	...
	expectedRootHashA := []byte{0xa2, 0xe5, 0x54, 0xac, 0xd6, 0x4f, 0x2e, 0xe2, 0x32, 0x42, 0x51, 0x75, 0xdf, 0xbc, 0x1f, 0xae, 0xb1, 0x71, 0x66, 0x98, 0xd2, 0x2e, 0x66, 0x41, 0x40, 0x66, 0xe2, 0x95, 0xc4, 0xe9, 0xef, 0x51}
	assert.Equal(t, expectedRootHashA, doctreeA.RootHash())

	expectedRootHashB := []byte{0x2, 0x31, 0x93, 0x9, 0xf4, 0xcb, 0x9d, 0xb6, 0x7a, 0x66, 0xae, 0x7d, 0xc3, 0x65, 0x2f, 0xc8, 0x72, 0x7d, 0xab, 0x7f, 0x13, 0xb8, 0xc0, 0x3e, 0x62, 0x90, 0x70, 0xd1, 0xc7, 0xd6, 0x20, 0x64}
...
func TestTree_GenerateNestedTreeCombinedSortedHashesProof(t *testing.T) {
	...
	expectedRootHashA := []byte{0xf0, 0xc3, 0x90, 0x45, 0xec, 0x27, 0x16, 0x86, 0xff, 0x24, 0x9, 0x3a, 0xdb, 0xab, 0x6d, 0xc1, 0xc6, 0x8a, 0x25, 0xa8, 0xe9, 0xa5, 0xeb, 0x1, 0x69, 0x8, 0xd2, 0xeb, 0xb, 0x35, 0x52, 0x8c}
	assert.Equal(t, expectedRootHashA, doctreeA.RootHash())

	expectedRootHashB := []byte{0x6c, 0x40, 0xe0, 0xca, 0x7, 0x1f, 0xbe, 0x78, 0x45, 0x7a, 0xd1, 0x2, 0x59, 0xbc, 0x7b, 0xec, 0x73, 0x49, 0x3a, 0x3c, 0x97, 0xcb, 0xd1, 0x41, 0xec, 0xfe, 0xa2, 0x7c, 0x80, 0x90, 0x63, 0xc9}
	assert.Equal(t, expectedRootHashB, doctreeB.RootHash())
...
func TestTree_GenerateWithRepeatedFields(t *testing.T) {
	...
	err = doctree.Generate()
	assert.Nil(t, err)
	expectedRootHash := []byte{0xf0, 0xc3, 0x90, 0x45, 0xec, 0x27, 0x16, 0x86, 0xff, 0x24, 0x9, 0x3a, 0xdb, 0xab, 0x6d, 0xc1, 0xc6, 0x8a, 0x25, 0xa8, 0xe9, 0xa5, 0xeb, 0x1, 0x69, 0x8, 0xd2, 0xeb, 0xb, 0x35, 0x52, 0x8c}
	assert.Equal(t, expectedRootHash, doctree.RootHash())
...
func TestTree_GenerateWithNestedAndRepeatedFields(t *testing.T) {
	...
	err = doctree.Generate()
	assert.Nil(t, err)
	expectedRootHash := []byte{0x7, 0x36, 0x3b, 0x85, 0xf1, 0x5, 0x6f, 0xdd, 0xe7, 0x6d, 0x93, 0xa3, 0x72, 0xa3, 0xbf, 0x17, 0x24, 0xa0, 0x25, 0x32, 0x2e, 0x8c, 0x3a, 0x69, 0x1c, 0xf3, 0xb8, 0xec, 0x10, 0x19, 0xde, 0x36}
	assert.Equal(t, expectedRootHash, doctree.RootHash())
...
func TestCreateRepeatedSortedProof(t *testing.T) {
	...
	fieldHash, err := CalculateHashForProofField(&proof, sha256Hash)
	rootHash := []byte{0xf0, 0xc3, 0x90, 0x45, 0xec, 0x27, 0x16, 0x86, 0xff, 0x24, 0x9, 0x3a, 0xdb, 0xab, 0x6d, 0xc1, 0xc6, 0x8a, 0x25, 0xa8, 0xe9, 0xa5, 0xeb, 0x1, 0x69, 0x8, 0xd2, 0xeb, 0xb, 0x35, 0x52, 0x8c}
	assert.Equal(t, rootHash, doctree.rootHash)
	valid, err := ValidateProofSortedHashes(fieldHash, proof.SortedHashes, rootHash, doctree.hash)
	assert.True(t, valid)
