...
func TestTree_Generate(t *testing.T) {
	...
	h := tree.Root().Hash
	expectedHash := []byte{0x2e, 0x47, 0x26, 0x10, 0xd9, 0x41, 0x29, 0x9a, 0x23, 0x90, 0xa0, 0x29, 0x68, 0xef, 0x2c, 0x50, 0xde, 0x6e, 0x8, 0x28, 0x1a, 0x60, 0xb, 0xad, 0xef, 0x98, 0x74, 0xd, 0x78, 0xa1, 0xde, 0xd9}
...
func TestSortedHashTree_Generate(t *testing.T) {
	...
	tree.Generate(hashes, sha256Hash)
	h := tree.Root().Hash
	expectedHash := []byte{0x95, 0x2b, 0x45, 0x2e, 0xc8, 0x51, 0xb1, 0x51, 0x14, 0xf5, 0x9, 0x43, 0x6b, 0x21, 0xc6, 0xef, 0x4f, 0xe0, 0x23, 0x6f, 0xed, 0xe5, 0xfd, 0xe2, 0xfc, 0x1d, 0xec, 0x94, 0xf9, 0xd0, 0xfc, 0xdc}
	assert.Equal(t, expectedHash, h, "Hash should match")
}
...
func TestCreateProof_sorted(t *testing.T) {
	...
	fieldHash, err := CalculateHashForProofField(&proof, sha256Hash)
	rootHash := []byte{0xd3, 0xb6, 0x6f, 0xb9, 0x4e, 0x14, 0x4b, 0xb3, 0xb3, 0x81, 0x7c, 0xa0, 0x78, 0x9a, 0xb4, 0x3, 0x7f, 0x21, 0x72, 0x54, 0xda, 0x49, 0xca, 0xa7, 0x95, 0x9f, 0x5b, 0x7f, 0xea, 0xc, 0x79, 0xdf}
	assert.Equal(t, rootHash, doctree.rootHash)
